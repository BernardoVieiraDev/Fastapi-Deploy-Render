...
async def populate_posts(db):
    ...
    service = PostService()
    await service.create_post(PostIn(title="post 1", content="some content", published=True))
    await service.create_post(PostIn(title="post 2", content="some content", published=True))
    await service.create_post(PostIn(title="post 3", content="some content", published=False))
